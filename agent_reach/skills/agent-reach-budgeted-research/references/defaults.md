...
- `実行モード`: use `narrow` for one clear lookup, `bounded_multi_source` for small comparisons, and `broad_with_ledger` only when the user explicitly asks for wide coverage or provenance-heavy work
...
- `証拠の残し方`: use `--save-dir .agent-reach/shards` for multi-command broad runs, then merge before `ledger summarize`, `ledger query`, or `plan candidates`; use `--save .agent-reach/evidence.jsonl` for a simpler single-ledger path
- `候補選別ゲート`: use `agent-reach plan candidates --by normalized_url --limit 20 --json` before deep reads unless the user explicitly wants a wider shortlist

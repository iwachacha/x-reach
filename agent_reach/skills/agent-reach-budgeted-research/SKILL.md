---
name: agent-reach-budgeted-research
description: Shape broad or provenance-heavy research asks into a bounded Agent Reach execution plan with explicit artifact, discovery, candidate, and deep-read budgets. Use only when the user explicitly asks to use Agent Reach or names this skill, and the task is broad enough that token or artifact size needs to be controlled before collection begins.
---

# Agent Reach Budgeted Research

Use this skill only when the user explicitly asks to use Agent Reach or names this skill, and the task is broad, multi-source, or provenance-heavy enough that collection needs explicit budget guardrails first. For ordinary lightweight searching, use the model's native browsing/search instead.

Turn the user's research request into one bounded execution plan that downstream Agent Reach collection can follow without inflating artifact size or synthesis cost.

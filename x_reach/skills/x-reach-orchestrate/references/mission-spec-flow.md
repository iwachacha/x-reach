...
7. Inspect `mission-result.json` summary fields: `ranked_candidates`, `filter_drop_counts`, `quality_reason_counts`, `topic_fit_reason_counts`, `topic_fit_dropped`, `topic_spread_status`, `coverage_target_gap`, `coverage_query_budget_exhausted`, `throttle_sensitive_errors`, and `throttle_guard_triggered`.
8. For a separate review shortlist, run `x-reach plan candidates --input .x-reach/missions/<run>/raw.jsonl --by post --limit 20 --max-per-author 2 --prefer-originals --drop-noise --json`; add `--sort-by quality_score` only when utility-sorted review helps and `--topic-fit PATH.json` only when caller-declared fit rules exist outside the mission spec.
9. Treat `quality_score`, `quality_reasons`, `sort_by`, `topic_fit`, and summary reason counts as deterministic utility diagnostics, not final selection or trust.

...
- include `x-reach plan candidates --input .x-reach/evidence.jsonl --by post --limit 20 --max-per-author 2 --prefer-originals --drop-noise --json`; add `--sort-by quality_score` only when utility-sorted review is useful and `--topic-fit PATH.json` only with caller-declared fit rules
- review `quality_score`, `quality_reasons`, `topic_fit`, and reason counts as shortlist diagnostics, not final judgment

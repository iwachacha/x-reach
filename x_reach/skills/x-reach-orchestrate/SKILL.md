...
6. If the path is a broad mission, read [references/mission-spec-flow.md](references/mission-spec-flow.md) before writing or running the mission spec.
7. Use [references/routing-guides.md](references/routing-guides.md) for source and channel hints that match the task type.
8. Start actual X Reach checks and collection in-session. Do not stop at a prompt.
...
- For broad, resumable, artifact-heavy requests, prefer a mission spec and `x-reach collect --spec` over improvised command chains.
...
- For mission runs, inspect `summary.quality_reason_counts`, `topic_spread_status`, and coverage diagnostics before deciding whether another bounded pass is needed.
...
- Mission-spec execution workflow: [references/mission-spec-flow.md](references/mission-spec-flow.md)

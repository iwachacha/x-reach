...
- `候補選別ゲート`: How the run shrinks collected items before deep reads, including whether `plan candidates` is used, the candidate limit, any caller-declared `topic_fit` rules, and how `quality_score` / `quality_reasons` / `topic_fit` are inspected without turning them into final judgment.
...
- Fill every field.
- Keep each field actionable enough to drive real X Reach commands.

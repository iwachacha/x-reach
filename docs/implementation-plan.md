...
Last refreshed: 2026-04-18 JST after `user_posts` parity implementation and targeted verification.
...
- The latest implementation pass added caller-declared topic-fit rules.
  ...
  - `uv run ruff check x_reach tests`: passed.
- The latest implementation pass added `user_posts` quality parity.
  - `user_posts` now accepts caller-declared `min_likes`, `min_retweets`, `min_views`, and optional `topic_fit` rules.
  - The same support is exposed through `collect --operation user_posts`, `posts`, `XReachClient.twitter.user_posts(...)`, batch plans, `channels --json`, and explicit mission query objects with `operation=user_posts`.
  - Timeline filters are client-side only and report deterministic `filter_drop_counts`, `filter_drop_reasons`, metric filter diagnostics, topic-fit summaries, dropped samples, and positive topic-fit reason counts.
  - No search-only `search_type`, hidden author deep read, query expansion, or Top/Latest fallback was added.
...
- The biggest remaining usability gap is now score and filter calibration. Candidate-review ordering has an explicit opt-in quality sort while first-seen order remains the default, broad-run pacing now has explicit controls plus diagnostics, caller-declared topic-fit rules can replace brittle query-token fallback, and account timelines now share the same explicit metric/topic-fit control surface.
...
| Caller-declared topic-fit rules | `adopt_now` | Implemented. Mission specs, `plan candidates --topic-fit`, and `user_posts` now share deterministic required/preferred/excluded/phrase/synonym matching with compact match/drop diagnostics. Active topic-fit rules take priority over query-token fallback where applicable. | Calibrate topic-fit and quality diagnostics with representative artifacts; do not add domain defaults or model judgment. |
...
Start the next implementation pass with filter calibration and soft-rescue primitives unless there is a newer maintainer decision. Keep broader thin/promo/non-English candidate examples in the follow-up queue.

A focused adoption record for the external field-review notes now lives in [field-review-improvement-plan.md](field-review-improvement-plan.md). That plan marks opt-in candidate quality sorting, explicit broad-run pacing, caller-declared topic-fit, and `user_posts` parity as completed; the next safe slice is filter calibration and soft-rescue primitives. It explicitly defers automatic `Top` to `Latest` fallback, topic clustering, and VLM/location inference until they pass a fresh proposal gate.
...
- Added `user_posts` parity for caller-declared metric filters and optional topic-fit filtering across adapter, CLI, SDK, batch, operation contracts, and explicit mission timeline queries.

...
- `x-reach collect --spec mission.json --dry-run --json`
...
- Mission `ranked.jsonl` and `mission-result.json` use the same deterministic quality reasons and aggregate counts as candidate planning. `diversity.require_topic_spread` remains separate quota logic over declared coverage topics; scoring only adds auditable supporting signals, and coverage topic matches are auxiliary to concrete, first-hand, observable evidence.
